...
const emit = defineEmits(['updateVolume', 'toggleMute'])
</script>

<template>
  <div class="volume-control">
    <button @click="emit('toggleMute')" class="icon-button">
    ...
    <input 
      ...
      step="0.01" 
      :value="volume" 
      @input="emit('updateVolume', parseFloat($event.target.value))" 
      class="volume-bar"
    />
  </div>
</template>
...
.volume-bar {
  width: 80px; /* Daha kısa bir genişlik */
  height: 5px;
  cursor: pointer;
